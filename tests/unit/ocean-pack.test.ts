...
const oceanPack = parseYaml(
  readFileSync(join(process.cwd(), "content", "packs", "ocean-animals.yaml"), "utf8")
) as ContentPack;
...
describe("ocean animals content pack", () => {
  it("contains the requested scene, object count, languages, and levels", () => {
    ...
    for (const object of validatedPack.objects) {
      expect(Object.keys(object.content.en.levels)).toEqual(["L0", "L1", "L2", "L3", "L4"]);
      expect(Object.keys(object.content["zh-Hans"].levels)).toEqual(["L0", "L1", "L2", "L3", "L4"]);
    }
  });
  ...
  it("provides Simplified Chinese romanization and generated audio targets for each included level", () => {
    ...
    for (const object of validatedPack.objects) {
      const levels = object.content["zh-Hans"].levels as Record<LearningLevel, { romanization?: string; audio?: { path: string } }>;
      for (const level of ["L0", "L1", "L2", "L3", "L4"] as const) {
        expect(levels[level]?.romanization).toBeTruthy();
        expect(levels[level]?.audio?.path).toContain("/assets/generated/ocean-animals/audio/");
      }
    }
  });
});
