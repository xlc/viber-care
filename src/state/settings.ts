...
export function sanitizeSettings(input: unknown): WordGardenSettings {
  ...
  return {
    mode: value.mode && modeSet.has(value.mode) ? value.mode : DEFAULT_SETTINGS.mode,
    selectedPackId:
      typeof value.selectedPackId === "string" && value.selectedPackId.length > 0
    ...
    customLanguageOrder:
      customLanguageOrder.length > 0 ? customLanguageOrder : DEFAULT_SETTINGS.customLanguageOrder,
    showRomanization:
...
export function loadSettings(storage: StorageLike | undefined): WordGardenSettings {
  if (!storage) {
    return DEFAULT_SETTINGS;
  }
...
export function saveSettings(storage: StorageLike | undefined, settings: WordGardenSettings): void {
  if (!storage) {
    return;
  }
...
export function resolveLanguageOrder(settings: WordGardenSettings): LanguageCode[] {
