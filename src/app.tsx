...
import { createFindRound, handleFindTap, type FindRound } from "./game/find-mode";
import {
  ...
  useEffect(() => {
    ...
    fetch("/catalog.generated.json", { cache: "no-store" })
      ...
      .catch((catalogError: unknown) => {
        if (isMounted) {
          setError(catalogError instanceof Error ? catalogError.message : "Catalog failed to load.");
        }
      });
...
function getPlacements(catalog: RuntimeCatalog, packId: string): PlacementWithObject[] {
...
function Icon({ name }: { name: "gear" | "volume" | "muted" | "close" | "up" | "down" }) {
  const common = {
    width: 22,
    height: 22,
    viewBox: "0 0 24 24",
...
function SequenceText({
  ...
  return (
    <div className="sequence-text" data-testid="word-tray">
      {sequence.map((item) => (
        <div className="word-line" key={`${item.requestedLanguage}-${item.text}`}>
...
function ModeSegment({
  ...
  return (
    <div className="mode-segment" aria-label="Game mode">
    ...
    <div className="settings-backdrop" role="presentation">
      ...
      >
        ...
        <div className="settings-section">
          <h3>Content Pack</h3>
          <div className="preset-list" aria-label="Content pack">
            {catalog.packs.map((pack) => (
              <button
                key={pack.id}
                type="button"
                className={settings.selectedPackId === pack.id ? "is-selected" : ""}
                aria-pressed={settings.selectedPackId === pack.id}
                data-testid={`pack-${pack.id}`}
                onClick={() => update({ selectedPackId: pack.id, mode: "explore" })}
              >
        ...
        <div className="settings-section">
          <h3>Mode</h3>
          <ModeSegment mode={settings.mode} onChange={(mode) => update({ mode })} />
        </div>

        <div className="settings-section">
          <h3>Level</h3>
          <div className="level-grid" aria-label="Learning level">
          ...
          <div className="preset-list">
            {LANGUAGE_ORDER_PRESETS.map((preset) => (
              <button
                key={preset}
                type="button"
                className={settings.languageOrderPreset === preset ? "is-selected" : ""}
        ...
        <div className="settings-section compact">
          <label>
            <input
              ...
              onChange={(event) => update({ showRomanization: event.currentTarget.checked })}
            />
...
export function App() {
  ...
  const selectedPackId = catalog?.packs.some((packOption) => packOption.id === settings.selectedPackId)
  ...
  const objects = useMemo(() => placements.map((placement) => placement.object), [placements]);
  const targetObject = useMemo(() => {
    if (!findRound) {
      return objects[0] ?? null;
    }
    return objects.find((object) => object.id === findRound.targetObjectId) ?? objects[0] ?? null;
  }, [findRound, objects]);

  useEffect(() => {
    saveSettings(typeof window === "undefined" ? undefined : window.localStorage, settings);
  }, [settings]);
  ...
  useEffect(() => {
    if (settings.mode === "story" && targetObject) {
      setToast({ kind: "story", sequence: buildLearningSequence(targetObject, settings) });
    } else if (settings.mode === "find" && targetObject) {
      setToast({ kind: "find", sequence: getFindPrompt(targetObject, settings) });
    } else if (toast.kind === "find" || toast.kind === "story") {
      setToast({ kind: "hello", sequence: [] });
    }
  }, [settings.mode, settings.activeLevel, settings.languageOrderPreset, settings.customLanguageOrder, targetObject]);
    ...
    if (settings.mode === "find" && targetObject && findRound) {
      ...
      if (result.isTarget) {
        ...
        window.setTimeout(() => {
          ...
          const nextTarget = objects.find((candidate) => candidate.id === nextRound.targetObjectId);
          setFindRound(nextRound);
          if (nextTarget) {
            setToast({ kind: "find", sequence: getFindPrompt(nextTarget, settings) });
          }
        }, 1400);
        return;
      }
  ...
  const backgroundStyle = scene.background.asset
    ? ({ backgroundImage: `url(${scene.background.asset.path})` } as JSX.CSSProperties)
    : undefined;
  ...
  return (
    <main className="app-shell" data-mode={settings.mode}>
      ...
      <section className="play-area" aria-label={sceneTitle}>
        <div className={`prompt-ribbon is-${toast.kind}`} data-testid="prompt">
          ...
          {promptSequence.length > 0 ? (
            <SequenceText sequence={promptSequence} showRomanization={settings.showRomanization} />
          ) : (
      ...
      <footer className="language-strip" aria-label="Active language order">
