...
import { resolveLanguageOrder, type WordGardenSettings } from "../state/settings";
...
export function getDefaultScene(catalog: RuntimeCatalog, packId = "garden") {
  ...
  return pack.scenes.find((scene) => scene.id === pack.defaultSceneId) ?? pack.scenes[0];
}

export function getSceneObjects(catalog: RuntimeCatalog, packId = "garden"): ObjectConcept[] {
  const pack = getPack(catalog, packId);
...
function levelFallbackOrder(level: LearningLevel): LearningLevel[] {
  ...
  const remainingLevels = LEARNING_LEVELS.filter((candidate) => !lowerLevels.includes(candidate));
  return [...lowerLevels, ...remainingLevels];
}

function firstAvailableLanguage(object: ObjectConcept): LanguageCode {
  const firstLanguage = Object.keys(object.content)[0] as LanguageCode | undefined;
...
export function getLevelPresentation(
  ...
  const resolvedLanguage = object.content[language] ? language : firstAvailableLanguage(object);
  ...
  for (const candidateLevel of levelFallbackOrder(level)) {
    const levelContent = languageContent.levels[candidateLevel] as LevelContent | undefined;
    if (levelContent) {
      return {
        requestedLanguage: language,
        resolvedLanguage,
        ...
        audioText: levelContent.audioText || levelContent.text || languageContent.fallbackText,
