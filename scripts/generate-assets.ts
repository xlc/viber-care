...
async function readContentPacks(): Promise<ContentPack[]> {
	...
	for (const fileName of packFiles) {
		...
		rawPacks.push(
			extname(fileName) === '.json' ? JSON.parse(raw) : parseYaml(raw),
		)
	}
...
function oceanBackgroundSvg(asset: AssetReference): string {
	const label = escapeXml(
		asset.alt ?? asset.placeholder?.label ?? 'Ocean scene',
	)
...
function gardenBackgroundSvg(asset: AssetReference): string {
	const label = escapeXml(
		asset.alt ?? asset.placeholder?.label ?? 'Garden scene',
	)
...
function backgroundSvg(asset: AssetReference): string {
	const prompt =
		`${asset.generationPrompt ?? ''} ${asset.alt ?? ''}`.toLowerCase()
...
function createWavTone(frequency = 523.25, durationSeconds = 0.38): Buffer {
	...
	for (let index = 0; index < samples; index += 1) {
		...
		const sample =
			Math.sin((2 * Math.PI * frequency * index) / sampleRate) * fade * 0.08
...
async function writeAsset(
	...
	if (asset.type === 'image' || asset.type === 'background') {
		...
		return {
			...
			reviewRequired: true,
			warning:
				'Placeholder SVG generated from content metadata; review or replace with final art.',
		}
	}
...
async function main() {
	...
	const entries = await Promise.all(
		assets.map((asset, index) => writeAsset(asset, index)),
	)
	const manifest = {
		generatedAt: new Date().toISOString(),
		entries: entries.filter((entry): entry is AssetManifestEntry =>
			Boolean(entry),
		),
	}
