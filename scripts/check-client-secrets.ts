...
const patterns = [
  ...
  { name: "OpenAI runtime endpoint", regex: /api\.openai\.com|\/v1\/chat\/completions|\/v1\/responses|\/v1\/images/g },
  { name: "Anthropic runtime endpoint", regex: /api\.anthropic\.com/g },
  { name: "Google API key", regex: /\bAIza[A-Za-z0-9_-]{20,}\b/g },
  { name: "OpenAI key-like environment variable name", regex: /\b(?:VITE_)?OPENAI_[A-Z0-9_]*KEY\b/g }
];
...
async function main() {
  ...
  if (findings.length > 0) {
    const detail = findings.map((finding) => `${finding.file}: ${finding.name}`).join("\n");
    throw new Error(`Potential client secret exposure found:\n${detail}`);
  }

  console.log(`Scanned ${files.length} client file(s); no client secrets found.`);
}
